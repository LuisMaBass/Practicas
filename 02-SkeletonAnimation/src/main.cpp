...
glm::mat4 modelMatrizmayow = glm::mat4(1.0f);
glm::mat4 modelMatrizcowbow = glm::mat4(1.0f);
glm::mat4 modelMatrizguardian = glm::mat4(1.0f);
glm::mat4 modelMatrizcyborg = glm::mat4(1.0f);
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// modelos animados
	modelcyborg.loadModel("../models/cyborg/animacion.fbx");
	modelcyborg.setShader(&shaderMulLighting);
	modelcowbow.loadModel("../models/cowbow/Character Running.fbx");
	modelcowbow.setShader(&shaderMulLighting);
	modelguardian.loadModel("../models/"); /// pendiente guardian 
	modelguardian.setShader(&shaderMulLighting);
	modelmayow.loadModel("../models/mayow/personaje.fbx");
...
void destroy() {
	...
	// elementos animados a incorporar
	// manejar apuntadores para manimpolar mas rapido la informacion
	modelcyborg.destroy();
	modelcowbow.destroy();
...
void applicationLoop() {
	...
	// ==========================POSISIONES QUE SE AGREGARON ====================
	modelMatrizcyborg = glm::translate(modelMatrizcyborg, glm::vec3(13.0, 0.0, 5.0));

	modelMatrizguardian = glm::translate(modelMatrizguardian, glm::vec3(13.0, 0.0, 5.0));

	modelMatrizmayow = glm::translate (modelMatrizmayow, glm::vec3(13.0,0.0,15.0));

	modelMatrizcowbow = glm::translate (modelMatrizcowbow, glm::vec3 (18,0.0,2000));
	...
	while (psi) {
		...
		*******************************************/
		...
		// ==============================AGREGADOS HOY ====================================
		glm::mat4 modelMatrizmayowRody =glm::mat4(modelMatrizmayow);
		modelMatrizmayowRody = glm::scale (modelMatrizmayowRody, glm::vec3(0.02));
		modelmayow.render(modelMatrizmayowRody);

		glm::mat4 modelMatrizcowbowRody =glm::mat4(modelMatrizcowbow);
		//modelMatrizcowbowRody = glm::scale (modelMatrizcowbowRody, glm::vec3(0.02));
		modelcowbow.render(modelMatrizcowbowRody);
		/*******************************************
		 * Skybox
		 *******************************************/
