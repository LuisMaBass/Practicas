...
Sphere	esfera2(50,50);
// modificacion agregamos un nuevo modelo de tipo espfera 
...
GLuint skyboxTextureID, textureSponge;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	screenWidth = width;
	screenHeight = height;
//controlores de pantalla
	glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3); // manejador de ventanas gestiona eventos
	glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 3);// debemos de poner una version minima y maxima 
	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);// 

	if (bFullScreen)
		window = glfwCreateWindow(width, height, strTitle.c_str(),
				glfwGetPrimaryMonitor(), nullptr);// crecimeinto de pantalla 
	else
		window = glfwCreateWindow(width, height, strTitle.c_str(), nullptr,
				nullptr); // validacion de creacion de ventana 
	...
	glfwSwapInterval(0); // que vaya a maxima velocidad 
				// back bufer lo que se manda al interior de la comutadora 
				// front buffer lo quue estamos viendo 
	glfwSetWindowSizeCallback(window, reshapeCallback);//si se cambia el tamaño de la ventana redimension 
	glfwSetKeyCallback(window, keyCallback); // manda a ejecutar cuando sucede un evento 
												// si surge un evento lo cacha el tecaldo 
	glfwSetCursorPosCallback(window, mouseCallback);
	glfwSetMouseButtonCallback(window, mouseButtonCallback); // botones del mause 
	glfwSetInputMode(window, GLFW_STICKY_KEYS, GL_TRUE); // modo de entrada 
	...
	glViewport(0, 0, screenWidth, screenHeight);// puerto de vista se ve cortada la ventada a partir de cierto punto
...
// como inicialiar un modelo y renderizar tarea no 3
	boxCesped.init();
	...
	esfera1.init();
	esfera1.setShader(&shaderMulLighting); // shader asociado y le pasamos la referencia 

	esfera2.init();
	esfera2.setShader(&shaderMulLighting);// shader estandar 
	esfera2.setPosition(glm::vec3(10.0f, 10.0f, 10.0f)); // puede ser nesesaria si solo es estable la posicion si no 
	// ir al apartado de graficar directamente 
	...
	Texture textureSponge("../Textures/water.jpg");
...
void destroy() {
	...
	glDeleteTextures(1, &textureSponge);
...
void applicationLoop() {
	...
	while (psi) {
		currTime = TimeManager::Instance().GetTime();// singled honest usalo una ves y usalo muchas vees 
		// bajo acoplamiento, que no haya dependencia entre clases 
			// 
		if(currTime - lastTime < 0.016666667){// frames por segundo 1/60 
			glfwPollEvents();
			continue;
		}
		lastTime = currTime;
		...
		psi = processInput(true); // teclas de salidad el prgorama T/F
		// meotodo que se cupa aa poder manejar los elementos del taclado 
		// joystiks codigo referente a ellos 
		...
		glm::mat4 projection = glm::perspective(glm::radians(45.0f),// se reomienda manejar el filoview a 45 grados 
				(float) screenWidth / (float) screenHeight, 0.01f, 100.0f);// relacion de aspecto ancho y alto de la ventana 
													// plano cercano y plano lejad 
		...
		*********************************************/
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D,textureSponge);
		shaderMulLighting.setInt ("texture1",0);
		esfera2.setScale(glm::vec3(5.0f,2.0f,5.0f));
		esfera2.render();

		/*******************************************
		// esfera con mallado includio 
		*********************************************/
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureWallID);
		shaderMulLighting.setInt("texture1", 0);
...
// ==================
int main(int argc, char **argv) { // iniciar el main principal de programa
	init(800, 700, "Window GLFW", false); // ancho alto titulo de la venta y si queremos ue tenga tamañaño completo 
	applicationLoop(); // SIEMRPE  va a estas ejecutando hasta que le demos una tecla en especifico 
	destroy(); // 
	return 1;
}